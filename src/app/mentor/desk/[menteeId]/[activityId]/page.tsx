...
export default function MenteeStepPage() {
  ...
  return (
    <div className="px-6 py-6 max-w-[900px]">
      ...
      {/* A review gate, straight to its own card. This used to say "open it from your worklist",
          which sent the reviewer back to a list of fifty rows to find the one already in front of
          them. `gateHere` is the register's answer; `gate` (the verb) only covers the two
          task-boundary workspaces. */}
      {gateHere && (
        <div
          className={`mt-4 rounded-xl border px-4 py-3.5 ${
            gateHere.state === "awaiting"
              ? "border-[#f0c2c2] bg-[#fdecec]/50"
        ...
        >
          ...
          <p className="text-[12px] text-slate-700 leading-relaxed mt-1.5">
            {gateHere.state === "not_submitted"
              ? "They have not submitted this step, so there is no card to decide yet."
              : gateHere.state === "decided"
                ? `Decided by ${gateHere.decidedBy ?? "a reviewer"}. The card carries the reasons that were sent back.`
                : "The card carries the brief they read, their submission, the checklist, and the reasons your decision sends back."}
          </p>
          <OpenCardLink gate={gateHere} className="mt-2.5" />
        </div>
      )}
      ...
      <section className="mt-6">
        ...
        <p className="text-[12.5px] text-slate-700 tabular-nums">
          ...
          {step.attemptsRemaining <= 0 && (
            <span className="text-[#a31d1d]"> · none left</span>
          )}
        </p>
      </section>
      ...
      {review ? (
        <section className="mt-6">
          ...
          {/* Wearing the provenance tag, like everywhere else a model's output appears: this is
              the AI's provisional verdict, not a mentor's decision. */}
          <MachineNote>
