...
function ReviewPanel({ review }: { review: Review }) {
  ...
  return (
    // Dotted violet outline: this is the model's grade, not a mentor's decision. The mentor's
    // sealed verdict renders below it with a solid band, and the difference is the point.
    <MachineNote className="space-y-4" label={`Generated by ${review.model} · not a mentor decision`}>
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          {/* The model's name moved into the provenance tag at the top of the note — one place,
              not two, and it now reads as attribution rather than a footnote. */}
        </div>
      </div>
      ...
      {/* No tint and no ring of its own — it already sits inside the dotted note, and a second
          violet panel nested in the first just read as two objects. */}
...
function AcceptanceChecklist({ criteria, values, layer1, onClose }: {
  ...
  return (
    // The rule strip (form 04): your output is measured against this. The double rule top and
    // bottom is the whole signal — "measured against" — and it is why this carries no fill, no
    // tick circles and no progress bar: a criterion is met or it is open, and the word says which.
      ...
      <div className="flex items-baseline gap-2">
        ...
        {onClose && (
          <button
            ...
            className="focus-ring ml-auto -mr-1 shrink-0 w-6 h-6 rounded-md flex items-center justify-center text-slate-400 hover:text-slate-700 hover:bg-slate-900/5 transition-colors cursor-pointer"
          >
            <Icon name="x" size={13} />
          </button>
        )}
        ...
        {criteria.map((c, i) => {
          ...
          return (
            ...
            >
              ...
              <i
                className={`not-italic font-mono text-[10px] text-right ${
                  ok ? "text-emerald-600" : failed ? "text-amber-600" : "text-slate-400"
                }`}
              >
                {ok ? "met" : failed ? "not met" : "open"}
              </i>
            </li>
          );
        })}
...
/**
 * How this screen gets its data, and what it is allowed to do with it.
 *
 * Defaults to the signed-in learner working their own step. A mentor reviewing it mounts the very
 * same screen against the mentee's data, read-only, with the decision in place of the submit bar —
 * because the learner's post-submission view already *is* a read-only rendering of this work, and
 * building a reviewer's paraphrase of it beside it would be a second description that drifts.
 */
export interface StepScreenSource {
...
export function StepScreen({
  ...
  useEffect(() => {
    ...
    // `source` is memoised by its caller, so this refetches when the mentor switches mentee even
    // if the step id happens to be identical. Leaving it out made the identity of the fetcher
    // invisible to the effect, which is how you serve one learner's work under another's name.
  }, [activityId, source]);
  ...
  return (
    ...
    >
      ...
      {/* The working sheet (form 06). The folded corner is the most valuable cue in the form
          language: one glance answers "is this mine?". Nothing the mentee cannot edit may wear
          one — which is why this is written here on the deliverable rather than added to the
          shared <Card>, that being the wrapper for a dozen things they cannot touch.
        ...
        <div className="mt-5">
          {readOnly ? (
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {nextStepId ? (
                <Link href={`${base}/${nextStepId}`} className="focus-ring inline-flex items-center gap-2 h-10 px-4 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-[13px] font-semibold tracking-tight no-underline shadow-[0_4px_14px_-4px_rgba(79,70,229,0.6)]">
        ...
        <div className="space-y-6">
                  ...
                  return (
                    <div key={h.submission.id} className="flex gap-3">
                      ...
                      <div className="min-w-0 flex-1 pb-1">
                        ...
                        {r?.feedback && (
                          <>
                            {/* Same provenance as the full rubric, without the box — a history row
                                is too small to carry a note, but the attribution still has to be here. */}
                            <MachineTag className="mt-1.5" label="Generated" />
                            <p className="text-[11.5px] text-slate-500 mt-1 leading-relaxed tracking-tight" style={{ textWrap: "pretty" }}>{r.feedback}</p>
                          </>
                        )}
          ...
          {passed && !busy && (
            <section className="rounded-xl bg-emerald-50/70 ring-1 ring-emerald-200/70 p-3.5">
              ...
              {nextStepId ? (
                <Link href={`${base}/${nextStepId}`} onClick={() => setFeedbackOpen(false)} className="focus-ring inline-flex items-center gap-2 h-10 px-4 rounded-lg bg-indigo-600 hover:bg-indigo-700 text-white text-[13px] font-semibold tracking-tight no-underline shadow-[0_4px_14px_-4px_rgba(79,70,229,0.6)]">
                  {nextTaskCode && nextTaskCode !== activity.taskCode ? "Next task" : "Next step"} <Icon name="arrowRight" size={15} />
                </Link>
              ) : (
