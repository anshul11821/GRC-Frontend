...
/** A centred "document sheet" placeholder — for CV / certificate style pages. */
export function SkeletonSheet({ className = "" }: { className?: string }) {
  return (
    <div className={`animate-pulse max-w-3xl mx-auto ${className}`}>
      <div className="rounded-2xl ring-1 ring-slate-200/70 bg-white p-8 space-y-4">
        ...
        {Array.from({ length: 6 }).map((_, i) => (
          <div key={i} className="space-y-2">
            ...
            <Skeleton className="h-2.5 w-full opacity-70" />
            <Skeleton className="h-2.5 w-5/6 opacity-70" />
          </div>
        ))}
      </div>
    </div>
